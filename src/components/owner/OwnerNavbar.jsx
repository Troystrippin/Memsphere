...
const OwnerNavbar = ({ profile, avatarUrl, isDarkMode, toggleDarkMode }) => {
  ...
  return (
    ...
    `}>
        ...
        <div className="flex justify-between items-center h-20">
              ...
              return (
                ...
                >
                  <Icon className={`w-4 h-4 ${
                    isActive && !isDarkMode ? 'text-blue-600' : 'text-white'
                  }`} />
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            ...
            <div ref={notificationRef} className="relative">
              ...
              {/* Notifications Dropdown - Dark Mode Support */}
              {showNotifications && (
                <div className={`absolute right-0 mt-3 w-96 rounded-2xl shadow-2xl overflow-hidden animate-slideDown ${
                  isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-100'
                }`}>
                  <div className={`p-4 border-b ${
                    isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gradient-to-r from-blue-50 to-indigo-50 border-gray-100'
                  }`}>
                  ...
                  <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                    {loading ? (
                      <div className="p-8 text-center">
                        ...
                        <p className={`mt-2 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Loading...</p>
                      </div>
                      ...
                      notifications.slice(0, 5).map((notification) => (
                        <div
                          ...
                          className={`
                            ...
                            ${!notification.is_read ? (isDarkMode ? 'bg-blue-900/30' : 'bg-blue-50/30') : ''}
                            ${isDarkMode ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-50 hover:bg-gray-50'}
                          `}
                        >
                          <div className={`flex-shrink-0 w-10 h-10 rounded-xl shadow-sm flex items-center justify-center ${
                            isDarkMode ? 'bg-gray-700' : 'bg-white'
                          }`}>
                            {getNotificationIcon(notification.type)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-medium mb-1 line-clamp-2 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                              {getNotificationText(notification)}
                            </p>
                            {notification.business?.name && (
                              <p className={`text-xs mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                {notification.business.emoji} {notification.business.name}
                              </p>
                            )}
                            <p className={`text-xs flex items-center ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                      ...
                      <div className="p-12 text-center">
                        <Bell className={`w-16 h-16 mx-auto mb-4 ${isDarkMode ? 'text-gray-600' : 'text-gray-200'}`} />
                        <p className={`font-semibold mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>All caught up!</p>
                  ...
                  {notifications.length > 0 && (
                    <div className={`p-3 border-t ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-100'}`}>
                      <button
                        onClick={() => {
            ...
            {/* User Menu - Dark Mode Support - MODIFIED: Profile button navigates to /owner-profile */}
            <div ref={dropdownRef} className="relative">
              ...
              {showDropdown && (
                <div className={`absolute right-0 mt-3 w-80 rounded-2xl shadow-2xl overflow-hidden animate-slideDown ${
                  isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-100'
                }`}>
                  {/* Header */}
                  ...
                  {profile?.business_name && (
                    <div className={`px-4 py-3 border-b ${
                      ...
                      <div className="flex items-center space-x-2 text-sm">
                        ...
                        <span className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>{profile.business_name}</span>
                      </div>
                    </div>
                  )}

                  {/* Menu Items - ONLY Profile Button */}
                  <div className="p-2">
                    {/* Profile Button - Navigates to Owner Profile */}
                    <button
                      onClick={() => handleNavigation('/owner-profile')}
                      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-300 group ${
                        isDarkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-blue-50'
                      }`}
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                        isDarkMode ? 'bg-gray-700 group-hover:bg-blue-600' : 'bg-blue-100 group-hover:bg-blue-600'
                      }`}>
                        <UserCircle className={`w-4 h-4 ${isDarkMode ? 'text-blue-400 group-hover:text-white' : 'text-blue-600 group-hover:text-white'}`} />
                      </div>
                      <div className="flex-1 text-left">
                        <span className="font-medium block">Profile</span>
                        <span className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>View and edit your owner profile</span>
                      </div>
                    </button>
                    ...
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                        isDarkMode ? 'bg-red-900/20 group-hover:bg-red-600' : 'bg-red-100 group-hover:bg-red-600'
                      }`}>
                        <LogOut className={`w-4 h-4 ${isDarkMode ? 'text-red-400 group-hover:text-white' : 'text-red-600 group-hover:text-white'}`} />
                      </div>
                      <div className="flex-1 text-left">
                        <span className="font-medium block">Logout</span>
                        <span className={`text-xs ${isDarkMode ? 'text-red-400' : 'text-red-400'}`}>Sign out of your account</span>
                      </div>
                    </button>
                  </div>
                </div>
      ...
      {mobileMenuOpen && (
        <div ref={mobileMenuRef} className={`md:hidden animate-slideDown shadow-xl ${
          isDarkMode ? 'bg-gray-800 border-t border-gray-700' : 'bg-white border-t border-gray-100'
        }`}>
          <div className="px-4 py-3 space-y-1">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <Icon className={`w-5 h-5 ${isActive ? (isDarkMode ? 'text-white' : 'text-blue-600') : (isDarkMode ? 'text-gray-500' : 'text-gray-500')}`} />
            ...
            <div className={`border-t my-2 pt-2 ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
              <button
                onClick={toggleDarkMode}
