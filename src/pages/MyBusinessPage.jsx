...
const MyBusinessPage = () => {
  ...
  // Only show pending when permit is uploaded AND waiting for verification
  const isPending =
    business?.permit_document && business?.verification_status === "pending";
  const isRejected = business?.verification_status === "rejected";
  // No permit uploaded yet
  const hasNoPermit = !business?.permit_document && !business?.permit_verified;
    ...
    >
      ...
      <div className="h-full pt-20 overflow-y-auto">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-12">
          ...
          {/* VERIFICATION BANNERS - Only show the appropriate one */}
          {/* PENDING VERIFICATION BANNER - Only when permit is uploaded and waiting for admin review */}
          {isPending && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* NO PERMIT BANNER - Only when no permit has been uploaded */}
          {hasNoPermit && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
